    ...
    for item in nav:
        ...
        if subnavs is not None:
            ...
            if navname == 'brands':
                ...
                print go
    if go != "":
        page = BeautifulSoup(requests.get(go).content, 'html.parser')
        for i in range(5,8):
            col1 = page.find(class_='item-'+str(i)).find_all('p')
            for p in col1:
                text = p.string
                if text is not None and len(text) > 1:
                    brands.append(text.strip())
...
def explore_top_menu():
    ...
    for item in nav:
        ...
        if subnavs is not None:
            navname = item.find('a').text.strip().lower()
            if navname == 'clothing' or navname == 'shoes and accessories':
                print (
                ":::::::::::::::::::::::::::::::::::::::::::::::In nav : " + navname + ":::::::::::::::::::::::::::::::::::::::::::::::::::::")
...
def grep(url,subcate):
    ...
    if not is_product_exists(identifier, site_id):
        ...
        print  price + "original :" + str(original_price)

        product_id = save_product(identifier,re.escape(name),product_type['men'],subcate,price,original_price,brand_id,site_id,url)
        try:
